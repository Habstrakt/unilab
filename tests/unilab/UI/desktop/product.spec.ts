...
test("Проверить работу кнопки 'в корзину' внутри карточки услуги", async() => {
...
test("Проверить переключение табов на странице карточки мед. услуги", async() => {
	await test.step("Перейти на страницу 'Медицинских услуг' в шапке сайта", async() => {
		await headerPage.medicalServicesLink.click();
	});
...
test("Проверить переключение табов на странице карточки анализы", async() => {
