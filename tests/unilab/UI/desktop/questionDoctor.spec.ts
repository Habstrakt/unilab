...
test("Проверить подсчет количества введенных символов в поле ввода текста вопросу доктору", async() => {
...
test("Проверить отображение подсказки поля ввода номера заказа вопрос доктору", async({page}) => {
	await test.step("Открыть страницу 'Скажите, доктор'", async() => {
		await headerPage.openQuestionDoctor();
	});
